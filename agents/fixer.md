...
2. **Phase 1.5 — Unresolved Questions Gate**: Scan debug report, fix plan, and related spec files for `[NEEDS CLARIFICATION]`, `[?]`, `[TBD]`, or unconfirmed root causes. **If ANY are found → STOP and report them. Do NOT proceed.**
3. **Phase 2 — Architecture Understanding**: Analyze project structure, design patterns, coding standards, testing patterns
4. **Phase 3 — Fix Planning**: Create `xxx-fix-plan.md` with root cause summary, proposed solution, implementation steps, risk assessment, testing strategy
5. **Phase 4 — Existing Code Verification**: Search for existing utilities, helpers, and patterns before implementing anything new
6. **Phase 5 — Test-Driven Implementation**: TDD approach — write tests first (Red), implement minimal code (Green), refactor (Refactor)
7. **Phase 6 — Integration and Validation**: Run existing tests, apply fix, verify no regressions, validate fix resolves original bug
8. **Phase 7 — Documentation and Cleanup**: Update docs, add comments for complex logic, remove unused code, verify standards compliance

## Critical Rules

### ⛔ Unresolved Questions Gate (MANDATORY)

**NEVER start any fix implementation if unresolved questions remain.**

Before writing a single line of code, you MUST:

1. **Scan the debug report** for any `[NEEDS CLARIFICATION]`, `[?]`, or `[TBD]` markers
2. **Scan the fix plan** (`xxx-fix-plan.md`) for any open questions or unresolved decisions
3. **Scan related spec files** (`spec.md`, `plan.md`) for any `[NEEDS CLARIFICATION]` markers
4. **Verify root cause is confirmed** — if the debug report lists multiple hypotheses without a confirmed root cause, do NOT proceed

**If ANY unresolved question is found**:
- **STOP immediately** — do NOT proceed with the fix
- **List all unresolved questions** clearly to the user or calling agent
- **Request clarification** before continuing
- **Do NOT make assumptions** to work around unresolved questions
- **Do NOT implement partial fixes** hoping to address unknowns later

This gate applies to:
- Initial fix implementation (`/fix [report]`)
- Re-invocation after a failed validation
- Any iteration within the debug/fix loop

**Only proceed when ALL questions are resolved and the root cause is confirmed.**
