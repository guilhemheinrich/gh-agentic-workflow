...
Two findings recorded in `modelo-broker-pa` on 2026-09-09 are the second kind:

- `GRV-validate-edit-overrides-runner-a4eb` — the runner resolved a compose
  project name no container carried. Every `check` returned 125, the agent read
  one "no running container" warning, then silence for the rest of the session.
- `GRV-make-lint-never-lints-d035` — `golangci-lint run ./tests/integration/`
  printed `0 issues.` and exited 0, because a build constraint excluded all 120
...
`modelo-broker-pa`'s own containers (golangci-lint 2.12.2 / go 1.26.3, ESLint
9.39.4), ruff in `ghcr.io/astral-sh/ruff:latest` (ruff 0.15.21).
...
A canary that passes has three usual causes, in order of frequency: the linter
never saw the file (§2), the target container is not the one holding the edited
bytes (`--doctor`), or the command cannot fail (§3's `gofmt -l`).
...
| ---------- | -------------------------------------------------- | ----------------------------- |
| Go         | `go list -f '{{len .GoFiles}} {{len .IgnoredGoFiles}}' ./pkg/dir/` | `GoFiles` is `0`     |
| Python     | `ruff check --show-files <path>`                   | empty output                  |
| TS / JS    | `eslint --print-config <path>`                     | prints `undefined`            |

Measured contrast for each: `0 120` on the broker's `tests/integration`,
`warning: No Python files found under the given path(s)` under `force-exclude`,
and `undefined` on `src/routeTree.gen.ts`.
...
`govet`'s printf check is on by default in golangci-lint, so this fails without
depending on the repo's `.golangci.yml` selection:
...
| `--build-tags` on a large tagged package | Correct, but slow: 6.9s on the broker's `tests/integration`, over a 3s budget. | `skip` and leave the tagged files to `make lint` and CI. |
| Linting one file instead of its directory | golangci-lint has no single-file mode. A lone file typechecks as a broken package — 7 false `typecheck` issues on `convert.go`, 2026-09-09. | `check golangci-lint run "./$(dirname "$F")/"`. |
| Vendored module without `GOFLAGS=-mod=mod` | The run dies on `inconsistent vendoring in /app` before analysing anything, listing every module and naming no line. The agent reads it as a violation of the file it just wrote. `GRV-routing-table-ships-factory-e300`, 2026-09-16; with the flag the same command returned 5 real findings. | Set `GOFLAGS` in the compose service env, or inline `env GOFLAGS=-mod=mod` in the `check`. |
...
Measured against the broker's admin SPA config:
...
Measured 2026-09-17 on Docker via OrbStack, against `broker-pa-broker-1` and a
...
`125` never appears in that table, and never will: the docker CLI reserves it
for its own usage errors, such as an unknown flag on `docker run` — a command
the hook does not call, so no test should assert that path. Every path the
runner can reach is above. The three infrastructure failures that matter all
...
`modelo-broker-pa` worktree on 2026-09-16, where an agent saw it twice, stopped
trusting the hook, and ran the linter by hand
(`GRV-stale-cached-container-reported-aa46`).

Two consequences for a routing table. Any branch output that names no line is
infrastructure, not a finding. And a canary that fails for the *wrong* reason
proves nothing — read the first line before concluding the branch bites.

The daemon-unreachable row comes from the aa46 investigation; the other four
were re-measured here directly.

## 7. Write down every deliberate blind spot
...
The broker's Go branch is the model:
...
#    GRV-make-lint-never-lints-d035 tracks the same gap in `make lint`.
```
