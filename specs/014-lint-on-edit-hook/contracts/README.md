...
| (Bats-driven, no fixture)        | Routed branch exits 65 → wiring-missing message surfaced verbatim.                                  | FR-014, SC-002    |
...
- The Makefile contract is **owned by this repo** — exit codes `64`/`65` are our convention, documented in [makefile-interface.md](./makefile-interface.md) and propagated by the `makefile-lint-router` skill. The dispatcher is the body of the `lint:` recipe itself; there is no separate routing script.
